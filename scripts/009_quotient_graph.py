...
def find_path_through_quotient(G, Q, start, end):
    ...
    for i in range(len(shortest_q_path) - 1):
        ...
        connecting_edges = Q[current_q_node][next_q_node]['connecting_edges']
        
        current_subgraph = G.subgraph(Q.nodes[current_q_node]['members'])
        target_nodes = set(node for edge in connecting_edges for node in edge if node in current_subgraph)
        
        if g_path[-1] not in current_subgraph:
            current_node = next(iter(set(current_subgraph.nodes()) & set(node_to_q_nodes[g_path[-1]])))
        else:
            current_node = g_path[-1]
        
        nearest_target = min(target_nodes, key=lambda x: nx.shortest_path_length(current_subgraph, current_node, x))
        subpath = nx.shortest_path(current_subgraph, current_node, nearest_target)
        
        g_path.extend(subpath[1:])
        q_nodes.extend([current_q_node] * (len(subpath) - 1))
        
        next_node = next(node for edge in connecting_edges for node in edge if node in Q.nodes[next_q_node]['members'])
        g_path.append(next_node)
        q_nodes.append(next_q_node)
        
        # 方向の判定
        if Q.nodes[current_q_node]['type'] == 'additional' and len(subpath) > 1:
            members = Q.nodes[current_q_node]['members']
            entry_index = members.index(subpath[0])
            exit_index = members.index(subpath[-1])
            if entry_index < exit_index:
                node_direction_dict[current_q_node] = 'forward'
            elif entry_index > exit_index:
                node_direction_dict[current_q_node] = 'backward'
            else:
                node_direction_dict[current_q_node] = 'unknown'
        else:
            node_direction_dict[current_q_node] = 'unknown'
    ...
    # 最後のQ-nodeの方向を判定
    if Q.nodes[shortest_q_path[-1]]['type'] == 'additional' and len(last_subpath) > 1:
        members = Q.nodes[shortest_q_path[-1]]['members']
        entry_index = members.index(last_subpath[0])
        exit_index = members.index(last_subpath[-1])
        if entry_index < exit_index:
            node_direction_dict[shortest_q_path[-1]] = 'forward'
        elif entry_index > exit_index:
            node_direction_dict[shortest_q_path[-1]] = 'backward'
        else:
            node_direction_dict[shortest_q_path[-1]] = 'unknown'
    else:
        node_direction_dict[shortest_q_path[-1]] = 'unknown'
    
    return g_path, q_nodes, node_direction_dict
...
# 左下から右上に向かっていく
# 最も左下のノードと最も右上のノードを取得
if additional_partition_size < max_additional_partition_size:
    most_left_down = P.index(min(P, key=lambda x: x[0] ** 2 + x[1] ** 2))
    most_right_up = P.index(min(P, key=lambda x: (1000 - x[0]) ** 2 + (1000 - x[1]) ** 2))
    path = nx.shortest_path(G, source=most_left_down, target=most_right_up)
    if len(path) + additional_partition_size < max_additional_partition_size:
        ...
        additional_partitions.append(path[(len(path)-length)//2:(len(path)-length)//2+length])
        additional_partition_size += length
# 左上から右下に向かっていく
# 最も左上のノードと最も右下のノードを取得
if additional_partition_size < max_additional_partition_size:
    most_left_up = P.index(min(P, key=lambda x: x[0] ** 2 + (1000 - x[1]) ** 2))
    most_right_down = P.index(min(P, key=lambda x: (1000 - x[0]) ** 2 + x[1] ** 2))
    path = nx.shortest_path(G, source=most_left_up, target=most_right_down)
    if len(path) + additional_partition_size < max_additional_partition_size:
        additional_partitions.append(path)
        additional_partition_size += len(path)
    else:
        ...
        additional_partition_size += length
# 中央左から中央右に向かっていく
# 最も中央左のノードと最も中央右のノードを取得
if additional_partition_size < max_additional_partition_size:
    most_left_center = P.index(min(P, key=lambda x: x[0] ** 2 + (500 - x[1]) ** 2))
    most_right_center = P.index(min(P, key=lambda x: (1000 - x[0]) ** 2 + (500 - x[1]) ** 2))
    path = nx.shortest_path(G, source=most_left_center, target=most_right_center)
    if len(path) + additional_partition_size < max_additional_partition_size:
        additional_partitions.append(path)
        additional_partition_size += len(path)
    else:
        ...
        additional_partition_size += length
# 中央上から中央下に向かっていく
# 最も中央上のノードと最も中央下のノードを取得
if additional_partition_size < max_additional_partition_size:
    most_up_center = P.index(min(P, key=lambda x: (500 - x[0]) ** 2 + (1000 - x[1]) ** 2))
    most_down_center = P.index(min(P, key=lambda x: (500 - x[0]) ** 2 + x[1] ** 2))
    path = nx.shortest_path(G, source=most_up_center, target=most_down_center)
    if len(path) + additional_partition_size < max_additional_partition_size:
        additional_partitions.append(path)
        additional_partition_size += len(path)
    else:
...
while additional_partition_size < max_additional_partition_size:
    path = nx.shortest_path(G, source=random.choice(list(G.nodes())), target=random.choice(list(G.nodes())))
    if len(path) + additional_partition_size < max_additional_partition_size:
        additional_partitions.append(path)
        additional_partition_size += len(path)
    else:
...
for pos_to in t:
    g_path, q_nodes, node_directions = find_path_through_quotient(G, Q, cur_node, pos_to)
    breakpoint()
    g_path = g_path[1:]
    q_nodes = q_nodes[1:]
    ...
    for i in range(len(g_path)):
        ...
        # 次のノードがAに含まれていない場合
            ...
            if node_type == "base":
                ...
                direction = node_directions[q_nodes[i]]
                if direction == "forward":
                    ...
                    if idx - L_B >= 0:
                        B = sub_A[idx-L_B+1:idx+1]
                        final_output.append(f"s {L_B} {start_idx+idx-L_B+1} 0")
                    else:
                        th = random.randint(0, L_B - idx)
                        B[th:th+idx+1] = sub_A[:idx+1]
                        final_output.append(f"s {idx} {start_idx+1} {th}")
