...
def generate_A(graph, l_a, P):
    ...
    if len(output) < l_a:
        ...
        most_right_up = P.index(max(P, key=lambda x: x[0] ** 2 + x[1] ** 2))
        path = nx.shortest_path(graph, source=most_left_down, target=most_right_up)
        if len(path) + len(output) <= l_a:
            output.extend(path)
        else:
            length = l_a - len(output)
            output.extend(path[(l_a-length)//2:l_a-(l_a-length)//2])
    # 左上から右下に向かっていく
    # 最も左上のノードと最も右下のノードを取得
    if len(output) < l_a:
        most_left_up = P.index(min(P, key=lambda x: x[0] ** 2 + (1000 - x[1]) ** 2))
        most_right_down = P.index(max(P, key=lambda x: (1000 - x[0]) ** 2 + x[1] ** 2))
        path = nx.shortest_path(graph, source=most_left_up, target=most_right_down)
        if len(path) + len(output) <= l_a:
            output.extend(path)
        else:
            length = l_a - len(output)
    ...
    if len(output) < l_a:
        ...
        most_right_center = P.index(max(P, key=lambda x: (1000 - x[0]) ** 2 + (500 - x[1]) ** 2))
        path = nx.shortest_path(graph, source=most_left_center, target=most_right_center)
        if len(path) + len(output) <= l_a:
            output.extend(path)
        else:
            length = l_a - len(output)
    ...
    if len(output) < l_a:
        ...
        most_down_center = P.index(max(P, key=lambda x: (500 - x[0]) ** 2 + x[1] ** 2))
        path = nx.shortest_path(graph, source=most_up_center, target=most_down_center)
        if len(path) + len(output) <= l_a:
            output.extend(path)
        else:
            length = l_a - len(output)
