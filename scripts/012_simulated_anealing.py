...
def move_node_between_subgraphs(G, subgraphs, max_size, n_iterations):
    for _ in range(n_iterations):
        # Find the smallest subgraph
        min_subgraph = min(subgraphs, key=len)

        # Check if the minimum size exceeds max_size
        if len(min_subgraph) > max_size:
            break

        # Find neighboring subgraphs
        neighbors = set()
        for node in min_subgraph:
            neighbors.update(G.neighbors(node))
        
        # Filter out neighbors that are in the min_subgraph itself
        neighbors = [n for n in neighbors if not any(n in sg for sg in subgraphs if sg != min_subgraph)]
        
        if not neighbors:
            continue
        
        # Select a random neighboring node
        selected_node = random.choice(neighbors)

        # Find the subgraph that contains this node
        for subgraph in subgraphs:
            if selected_node in subgraph:
                selected_subgraph = subgraph
                break
        
        # Remove the selected node from its original subgraph
        # and add it to the smallest subgraph
        temp_subgraph = selected_subgraph.copy()
        temp_subgraph.remove(selected_node)
        
        # Check if removing the node breaks the connectivity of the original subgraph
        if nx.is_connected(G.subgraph(temp_subgraph)):
            selected_subgraph.remove(selected_node)
            min_subgraph.add(selected_node)
        else:
            continue  # Skip this iteration if it breaks connectivity

    # Return the modified subgraphs
    return subgraphs
...
if not debug:
    ...
    with open("./sample_input/0074.txt", "r") as f:
...
additional_partitions = [set(part) for part in additional_partitions]
# グラフを分割
partitions = partition_graph(G, P, L_B)
partitions = [set(part) for part in partitions]
# quotient graphを構築
...
best_costs = len([l for l in final_output if l[0] == "s"])
best_partitions = partitions.copy()
best_outputs = final_output.copy()

current_costs = best_costs
current_partitions = partitions.copy()
current_outputs = final_output.copy()

initial_temp = 2
cooling_rate = 0.95

tempreture = initial_temp

while time.time() - start < 2.4:

    partitions = move_node_between_subgraphs(G, current_partitions, L_B, 10)
    Q = custom_quotient_graph(G, partitions, additional_partitions)
    ...
    for pos_to in t:
        ...
        costs = len([l for l in final_output if l[0] == "s"])
        if costs < current_costs or random.random() < np.exp((current_costs - costs) / initial_temp):
            current_costs = costs
            current_partitions = partitions.copy()
            current_outputs = final_output.copy()
            if costs < best_costs:
