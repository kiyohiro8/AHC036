...
def evaluate_output(actual_output, input_path):
    """
    実際の出力と期待される出力を比較する
    """
...
def evaluate_script(script_path, input_dir):
    """
    スクリプトと入力を用いて、スクリプトの動作を検証する
    """
    paths = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if os.path.isfile(os.path.join(input_dir, f)) and f.startswith("0") and f.endswith(".py")]
    all_passed = True
    for input_path in paths:
        ...
        checked_output = evaluate_output(actual_output, input_path)
        if checked_output:
...
if __name__ == "__main__":
    ...
    column_names = ["script"]
    for i in range(50):
        column_names.append(f"{i:04}.txt")
    if os.path.exists("evaluation.csv"):
        evaluation_df = pd.read_csv("evaluation.csv", index_col=0)
    else:
        evaluation_df = pd.DataFrame(columns=column_names)

    for script in script_files:
        if script in evaluation_df.index:
            print(f"Already evaluated: {script}")
            continue
        for input_file in input_files:
            script_path = os.path.join(script_dir, script)
            input_path = os.path.join(input_dir, input_file)
            result = run_script_with_input(script_path, input_path)
            # コスト計算
            cost = len([l for l in result.split("\n") if l.startswith("s")])
            evaluation_df.loc[script, input_file] = cost
